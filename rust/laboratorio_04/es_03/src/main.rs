...
impl Clone for Node {
    fn clone(&self) -> Self {
        match self {
            Node::File(f) => {Node::File(RefCell::new(File::from(f.borrow())))}
            Node::Dir(d) => {Node::Dir(RefCell::new(Dir::from(d.borrow())))}
        }
    }
}
impl Node{
    ...
    fn name(&self) -> String{
        match self{
            Node::Dir(E) => {E.borrow().name.clone()},
            Node::File(E) => {E.borrow().name.clone()},
        }
    }
    fn content(&self) -> Option<Vec<u8>>{
        match self{
            Node::File(E) => {Some(E.borrow().content.clone())}
            Node::Dir(E) => None
        }
    }
    fn creation_time(&self) -> u64{
        match self{
            Node::File(E) => {E.borrow().creation_time.clone()}
            Node::Dir(E) => {E.borrow().creation_time.clone()}
        }
    }
    ...
    fn get_mut_file(node: &mut Node) -> Option<&mut File>{
        match node{
            Node::Dir(E) => None,
            Node::File(E) => Some(E.get_mut()),
        }
    }
    fn get_mut_dir(node: RefMut<Node>) -> Option<RefMut<Dir>>{
        match node{
            Node::Dir(E) => Some(E.borrow_mut()),
            Node::File(E) => None,
        }
    }
    fn get_file(node: &Node) -> Option<Ref<File>>{
        match node{
            Node::Dir(E) => None,
            Node::File(E) => Some(E.borrow())
        }
    }

    fn get_dir(node: &Node) -> Option<Ref<Dir>>{
        match node{
            Node::File(E) => None,
            Node::Dir(E) => Some(E.borrow())
        }
    }
...
impl FileSystem{
    ...
    pub fn new(fs_name: &str) -> FileSystem{
        ...
        let mut new_file_system = FileSystem{
            root: RefCell::new(Dir::new(fs_name,0,vec![]))
        };
        new_file_system
    }
    /*
    ...
    /// CREATE DIRECTORY
    fn mk_dir_r(path: &str, mut current_loc: RefMut<Dir>){
        println!("{:?}",path);
        /*
        if path.len() == 0{
            return true;
        ...
        if split_path.len() == 1{
           ...
           //for i in 0..(*current_loc).children.len(){
            for i in 0..(current_loc.borrow()).children.len(){
            ...
            (current_loc.borrow_mut()).children.push(new_node);
            return;
        }
       ...
       // for i in 0..(*current_loc).children.len(){
        for i in 0..(current_loc.borrow()).children.len(){
            ...
            //match &mut (*current_loc).children[i]{
            match &(current_loc.borrow()).children[i]{
                Node::Dir(e) => {
                    if e.borrow().name == split_path[0]{
                        Self::mk_dir_r(split_path[1..].join("/").as_str(),e.borrow_mut());
                    }
                }
                _ => {}
            }
    ...
    pub fn mk_dir(&mut self,path: &str){
        ...
        if new_path[0] != self.root.borrow().name{
            //println!("Invalid file system name");
            return;
        }

        Self::mk_dir_r(new_path[1..].join("/").as_str(),self.root.borrow_mut());
    ...
    fn rm_dir_r(path: &str,mut  current_loc: RefMut<Dir>){
        println!("{:?}",path);
        ...
        //for i in 0..(current_loc.borrow()).children.len(){
            ...
            match &(current_loc.borrow()).children[i]{
                Node::Dir(e) => {
                    if e.borrow_mut().name == split_path[0] && split_path.len() == 1{
                        if e.borrow().children.len() == 0{
                    ...
                    else if e.borrow().name == split_path[0]{
                        Self::rm_dir_r(split_path[1..].join("/").as_str(),e.borrow_mut());
                    }
                }
                _ => {}
            }
    ...
    pub fn rm_dir(&self,path: &str){
        ...
        /// check if root is correct
        if new_path.len() == 1 && new_path[0] == self.root.borrow().name{
        ...
        Self::rm_dir_r(new_path[1..].join("/").as_str(),self.root.borrow_mut());
        //println!("Returning false as nothing was matched in recursive call");
        return;
    }

    /// INSERT FILE IN DIRECTORY
    fn new_file_r(path: &str, mut current_loc: RefMut<Dir>, file: &File){
        ...
        if path.contains("/"){
            ...
            for i in 0..(current_loc.borrow()).children.len(){
                ...
                match &(current_loc.borrow()).children[i]{
                    Node::Dir(e) => {
                        if e.borrow().name == split_path[0]{
                            Self::new_file_r(split_path[1..].join("/").as_str(),e.borrow_mut(),file);
                        }
                    }
                    _ => {}
                }
           ...
           // (*current_loc).children.push(Node::File(
            (current_loc.borrow_mut()).children.push(Node::File(
                RefCell::from(File {
    ...
    pub fn new_file(&mut self,path: &str, file: File){
        ...
        if new_path[0] != self.root.borrow().name || new_path.len() <= 1{
            return;
        }

        Self::new_file_r(new_path[1..].join("/").as_str(),self.root.borrow_mut(),&file);
        return;
    }
    fn rm_file_r(path: &str, mut current_loc: RefMut<Dir>){
        println!("{:?}",path);
        ...
        for i in 0..(current_loc.borrow()).children.len(){
            ...
            match &(current_loc.borrow()).children[i]{
                ...
                Node::Dir(e) => {
                    if e.borrow().name == split_path[0] && split_path.len() == 2{
                        /// if you only have the file remaining in the path, check between the current directory's children and remove that file
                        for x in 0..e.borrow().children.len(){
                            match &e.borrow().children[x]{
                                Node::File(e_1) => {
                                    if e_1.borrow().name == split_path[1]{
                                        e.borrow_mut().children.remove(x);
                        ...
                        Self::rm_file_r(split_path[1..].join("/").as_str(),e.borrow_mut());
                    }
                }
                _ => {}
            }
    ...
    pub fn rm_file(&mut self,path: &str){
        ...
        Self::rm_file_r(new_path[1..].join("/").as_str(),self.root.borrow_mut());
        return;
    }
    ...
    fn get_file_r<'a>(path: &str, mut current_loc : RefMut<Dir>) -> Option<Ref<'a,File>>{
        if path.contains("/"){
            let split_path: Vec<&str> = path.split("/").collect();
            ...
            for i in 0..current_loc.children.len(){
                ...
                if current_loc.children[i].is_dir() && current_loc.children[i].name().eq(split_path[0]){
                    ...
                    return Self::get_file_r(new_path.as_str(),Node::get_mut_dir(&current_loc.borrow().children[i]).unwrap());
            ...
            for i in 0..current_loc.children.len(){
                if current_loc.children[i].is_dir() == false && Node::get_mut_file(&mut current_loc.children[i]).unwrap().name == path{
                    return Node::get_file(&current_loc.borrow().children[i])
                }
            }
        }
        // nothing found
    ...
    pub fn get_file(&mut self,path: &str,) -> Option<Ref<File>>{
        ...
        let result = Self::get_file_r(path_f.as_str(),self.root.borrow());
